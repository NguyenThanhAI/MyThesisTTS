...
class GradTTS(BaseModule):
    def __init__(self, n_vocab, n_spks, spk_emb_dim, n_enc_channels, filter_channels, filter_channels_dp, 
        ...
        if n_spks > 1:
            self.spk_emb = torch.nn.Embedding(num_embeddings=n_spks, embedding_dim=spk_emb_dim)
    ...
    def compute_loss(self, x, x_lengths, y, y_lengths, spk=None, out_size=None):
        ...
        diff_loss, xt = self.decoder.compute_loss(x0=y, mask=y_mask, mu=mu_y, spk=spk)
...
class GradTTSWithSpeakerEmbedding(BaseModule):
    def __init__(self, n_vocab, n_spks, spk_emb_dim, n_enc_channels, filter_channels, filter_channels_dp, 
                 n_heads, n_enc_layers, enc_kernel, enc_dropout, window_size, 
                 n_feats, dec_dim, beta_min, beta_max, pe_scale):
        super(GradTTSWithSpeakerEmbedding, self).__init__()
        self.n_vocab = n_vocab
        self.n_spks = n_spks
        self.spk_emb_dim = spk_emb_dim
        self.n_enc_channels = n_enc_channels
...
class ConsistencyModelWithSpeakerEmbeddingAdditive(BaseModule):
    def __init__(
            self,
            n_vocab: int,
            n_feats: int,
        ...
        super(ConsistencyModelWithSpeakerEmbeddingAdditive, self).__init__()
        self.n_vocab = n_vocab
        self.n_feats = n_feats
        self.n_enc_channels = n_enc_channels
        self.filter_channels = filter_channels
