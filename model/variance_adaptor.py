...
from model.base import StyleAdaptiveLayerNorm
...
class StyleVariancePredictor(BaseModule):
    def __init__(self, 
                 in_channels: int, 
                 filter_channels: int, 
                 kernel_size: int, 
                 p_dropout: float,
                 spk_emb_dim: int):
        super(StyleVariancePredictor, self).__init__()
        ...
        self.conv_1 = torch.nn.Conv1d(in_channels=in_channels, 
                                      ...
                                      kernel_size=kernel_size, 
                                      padding=kernel_size//2)
        self.saln1 = StyleAdaptiveLayerNorm(
            in_channel=filter_channels, 
            style_dim=spk_emb_dim
        )
        self.norm_1 = LayerNorm(channels=filter_channels)
        self.conv_2 = torch.nn.Conv1d(in_channels=filter_channels, 
                                      out_channels=filter_channels, 
                                      kernel_size=kernel_size, 
                                      padding=kernel_size//2)
        self.saln2 = StyleAdaptiveLayerNorm(
            in_channel=filter_channels, 
            style_dim=spk_emb_dim
        )
        self.norm_2 = LayerNorm(channels=filter_channels)
    ...
    def forward(self, x, x_mask, spk_emb):
        x = self.conv_1(x * x_mask)
        x = torch.relu(input=x)
        x = self.saln1(x, spk_emb)
        x = self.norm_1(x)
        x = self.drop(x)
        x = self.conv_2(x * x_mask)
        x = torch.relu(input=x)
        x = self.saln2(x, spk_emb)
        x = self.norm_2(x)
        x = self.drop(x)
        x = self.proj(x * x_mask)
...
class VarianceAdaptor(BaseModule):
    def __init__(self,
        ...
        with open(stats_file_path, "r") as f:
            stats = json.load(f)
            pitch_min = stats["pitch"][0] #["min"]
            pitch_max = stats["pitch"][1] #["max"]
            energy_min = stats["energy"][0] #["min"]
            energy_max = stats["energy"][1] #["max"]
...
class StyleVarianceAdaptor(BaseModule):
    def __init__(self,
                 stats_file_path: str,
                 in_channels: int=256,
                 filter_channels: int=256,
        ...
        super(StyleVarianceAdaptor, self).__init__()
        ...
        self.duration_predictor = StyleVariancePredictor(in_channels=in_channels,
                                                          filter_channels=filter_channels,
                                                          kernel_size=kernel_size,
                                                          p_dropout=p_dropout,
                                                          spk_emb_dim=spk_emb_dim)

        self.pitch_predictor = StyleVariancePredictor(in_channels=in_channels,
                                                      filter_channels=filter_channels,
                                                      kernel_size=kernel_size,
                                                      p_dropout=p_dropout,
                                                      spk_emb_dim=spk_emb_dim)
        
        self.energy_predictor = StyleVariancePredictor(in_channels=in_channels,
                                                       filter_channels=filter_channels,
                                                       kernel_size=kernel_size,
                                                       p_dropout=p_dropout,
                                                       spk_emb_dim=spk_emb_dim)
