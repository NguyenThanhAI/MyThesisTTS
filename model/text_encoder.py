...
class ConvReluNorm(BaseModule):
    def __init__(self, in_channels, hidden_channels, out_channels, kernel_size, 
        ...
        self.conv_layers.append(torch.nn.Conv1d(in_channels=in_channels, 
                                                out_channels=hidden_channels, 
                                                kernel_size=kernel_size, 
                                                padding=kernel_size//2))
        self.norm_layers.append(LayerNorm(channels=hidden_channels))
        self.relu_drop = torch.nn.Sequential(torch.nn.ReLU(), torch.nn.Dropout(p=p_dropout))
        for _ in range(n_layers - 1):
            self.conv_layers.append(torch.nn.Conv1d(in_channels=hidden_channels, 
                                                    ...
                                                    padding=kernel_size//2))
            self.norm_layers.append(LayerNorm(channels=hidden_channels))
        self.proj = torch.nn.Conv1d(in_channels=hidden_channels, 
...
class StyleDurationPredictor(BaseModule):
    def __init__(self, in_channels, filter_channels, kernel_size, spk_emb_dim,
                 p_dropout):
        super(StyleDurationPredictor, self).__init__()
        self.in_channels = in_channels
        self.filter_channels = filter_channels
        ...
        self.conv_1 = torch.nn.Conv1d(in_channels=in_channels, 
                                      ...
                                      kernel_size=kernel_size, 
                                      padding=kernel_size//2)
        self.saln_1 = StyleAdaptiveLayerNorm(in_channel=filter_channels, style_dim=spk_emb_dim)
        self.conv_2 = torch.nn.Conv1d(in_channels=filter_channels, 
                                      out_channels=filter_channels, 
                                      kernel_size=kernel_size, 
                                      padding=kernel_size//2)
        self.saln_2 = StyleAdaptiveLayerNorm(in_channel=filter_channels, style_dim=spk_emb_dim)
        self.proj = torch.nn.Conv1d(in_channels=filter_channels, 
                                    out_channels=1, 
                                    kernel_size=1)
...
class MultiHeadAttention(BaseModule):
    def __init__(self, channels, out_channels, n_heads, window_size=None, 
        ...
        self.conv_q = torch.nn.Conv1d(in_channels=channels, out_channels=channels, kernel_size=1)
        self.conv_k = torch.nn.Conv1d(in_channels=channels, out_channels=channels, kernel_size=1)
        self.conv_v = torch.nn.Conv1d(in_channels=channels, out_channels=channels, kernel_size=1)
        if window_size is not None:
            n_heads_rel = 1 if heads_share else n_heads
            rel_stddev = self.k_channels**-0.5
            self.emb_rel_k = torch.nn.Parameter(data=torch.randn(n_heads_rel, window_size * 2 + 1, self.k_channels) * rel_stddev)
            self.emb_rel_v = torch.nn.Parameter(data=torch.randn(n_heads_rel, window_size * 2 + 1, self.k_channels) * rel_stddev)
        self.conv_o = torch.nn.Conv1d(in_channels=channels, out_channels=out_channels, kernel_size=1)
        self.drop = torch.nn.Dropout(p=p_dropout)

        torch.nn.init.xavier_uniform_(tensor=self.conv_q.weight)
        torch.nn.init.xavier_uniform_(tensor=self.conv_k.weight)
        if proximal_init:
            self.conv_k.weight.data.copy_(src=self.conv_q.weight.data)
            self.conv_k.bias.data.copy_(src=self.conv_q.bias.data)
        torch.nn.init.xavier_uniform_(tensor=self.conv_v.weight)
    ...
    def attention(self, query, key, value, mask=None):
        ...
        if self.window_size is not None:
            ...
            key_relative_embeddings = self._get_relative_embeddings(relative_embeddings=self.emb_rel_k, length=t_s)
            rel_logits = self._matmul_with_relative_keys(x=query, y=key_relative_embeddings)
            rel_logits = self._relative_position_to_absolute_position(x=rel_logits)
            scores_local = rel_logits / math.sqrt(self.k_channels)
            scores = scores + scores_local
        if self.proximal_bias:
            assert t_s == t_t, "Proximal bias is only available for self-attention."
            scores = scores + self._attention_bias_proximal(length=t_s).to(device=scores.device, 
                                                                           dtype=scores.dtype)
        if mask is not None:
        ...
        if self.window_size is not None:
            relative_weights = self._absolute_position_to_relative_position(x=p_attn)
            value_relative_embeddings = self._get_relative_embeddings(relative_embeddings=self.emb_rel_v, length=t_s)
            output = output + self._matmul_with_relative_values(x=relative_weights, 
                                                                y=value_relative_embeddings)
...
class Encoder(BaseModule):
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, 
        ...
        for _ in range(self.n_layers):
            ...
            self.ffn_layers.append(FFN(in_channels=hidden_channels, 
                                       out_channels=hidden_channels,
...
class TextEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, 
        ...
        self.encoder = Encoder(hidden_channels=n_channels + (spk_emb_dim if n_spks > 1 else 0), 
                               filter_channels=filter_channels, 
                               n_heads=n_heads, 
                               n_layers=n_layers, 
                               kernel_size=kernel_size, 
        ...
        self.proj_m = torch.nn.Conv1d(in_channels=n_channels + (spk_emb_dim if n_spks > 1 else 0), 
                                      out_channels=n_feats, 
                                      kernel_size=1)
        self.proj_w = DurationPredictor(in_channels=n_channels + (spk_emb_dim if n_spks > 1 else 0), 
...
class StyleTextEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, 
        ...
        super(StyleTextEncoder, self).__init__()
        self.n_vocab = n_vocab
        self.n_feats = n_feats
        self.n_channels = n_channels
        self.filter_channels = filter_channels
        ...
        self.encoder = StyleEncoder(hidden_channels=n_channels,
                                    filter_channels=filter_channels,
                                    n_heads=n_heads, 
                                    n_layers=n_layers, 
                                    kernel_size=kernel_size, 
...
class StyleUniversalTextFeatureEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, 
                 n_channels, filter_channels, n_heads, 
                 n_layers, kernel_size, 
                 p_dropout, window_size=None, spk_emb_dim=64):
        
        super(StyleUniversalTextFeatureEncoder, self).__init__()
        self.n_vocab = n_vocab
        self.n_feats = n_feats
        self.n_channels = n_channels
        self.filter_channels = filter_channels
